...
viz_wlp_years <- function(start_season, end_season, fran_tm = "franchise") {
  ...
  if (team_count == 1 | team_count == 3 | team_count == 5) {
    viz_col <- 1
  } else if (team_count == 2 | team_count == 4 | team_count == 6) {
    viz_col <- 2
  } else if (team_count > 6 & team_count < 10) {
    viz_col <- 3
  ...
  teams_meta$primary[
    teams_meta$TeamName == "Philadelphia Athletics"
  ] <- "#150360"
  teams_meta$secondary[
    teams_meta$TeamName == "Philadelphia Athletics"
  ] <- "#FFFFFF"
  teams_meta$primary[
    teams_meta$TeamName == "Kansas City Athletics"
  ] <- "#00843D"
  teams_meta$secondary[
    teams_meta$TeamName == "Kansas City Athletics"
  ] <- "#FFFFFF"
  ...
  teams_meta$secondary[
    teams_meta$TeamName == "Tampa Bay Devil Rays"
  ] <- "#D2BC50"
  teams_meta$primary[
  ...
  teams_meta$secondary[
    teams_meta$TeamName == "Cleveland Guardians"
  ] <- "#E50022"
  ...
  # Preserve one row per team for color fallbacks
  ...
  purrr::map(teams_factor, function(x) {
    ...
    teams_data |>
      ...
      highcharter::hc_add_series(
        teams_data[
          teams_data$ended == "Wild Card" & complete.cases(teams_data),
        ],
        type = "scatter",
        highcharter::hcaes(x = Season, y = WLpct),
        marker = list(
      ...
      highcharter::hc_add_series(
        teams_data[
          grepl("^Won", teams_data$ended) & complete.cases(teams_data),
        ],
        type = "scatter",
        highcharter::hcaes(x = Season, y = WLpct),
        marker = list(
      ...
      highcharter::hc_add_series(
        teams_data[
          grepl("L Champs$", teams_data$ended) & complete.cases(teams_data),
        ],
        type = "scatter",
        highcharter::hcaes(x = Season, y = WLpct),
        marker = list(
      ...
      highcharter::hc_add_series(
        teams_data[
          teams_data$ended == "WS Champs" & complete.cases(teams_data),
        ],
        type = "scatter",
        highcharter::hcaes(x = Season, y = WLpct),
        color = "blue",
      ...
      highcharter::hc_title(
        text = paste0(
          x,
          "<strong><span style='font-size: 20px; color: black </span></strong>"
        )
      ) |>
      ...
      highcharter::hc_credits(
        enabled = TRUE,
        text = paste0(
          "Source: Fangraph. Using 'bbraphsR' package. Retreived on: ",
