...
#### Get the players from Dominican Republic and saves it as a dataframe in the environmernt, called `rd`
source("scripts/rd_players_fixed.R")

#### Get the game logs of the debut for each Dominican player
rd_debut_game <- rd |>
  mutate(To = From) |>
...
#### Players dropped from rd (no game logs)
players_no_game_logs <- setdiff(rd$Name, rd_debut_game$Name)

rd[rd$Name %in% players_no_game_logs, c(2, 30)]

#### Create column with current teams so historical teams are compatible with {mlbplotr} package

rd_debut_game <- rd_debut_game |>
  mutate(
    Team_clean = case_when(
      # Formatting/normalization of current codes
      Team == "CHW" ~ "CWS",
      Team == "KCR" ~ "KC",
      Team == "SDP" ~ "SD",
      Team == "SFG" ~ "SF",
      ...
      Team == "KCA" ~ "ATH", # Kansas City Athletics → Oakland Athletics
      Team == "OAK" ~ "ATH", # Oakland Athletics → Oakland Athletics
      Team == "NYG" ~ "SF", # New York Giants → San Francisco Giants
      Team == "MLN" ~ "ATL", # Milwaukee Braves → Atlanta Braves
      Team == "SEP" ~ "MIL", # Seattle Pilots → Milwaukee Brewers
...
#### Summarize number of Dominican players that debuted by team, and list them ----
team_counts_complete <- rd_debut_game |>
...
whole_plot <- team_counts_complete %>%
  ...
  labs(
    title = "Debut de Dominicanos en MLB",
    subtitle = "Jugadores por franquicia",
    x = NULL,
    y = NULL,
    caption = sprintf(
      "Equipos antiguos considerados en sus franquicias actuales.<br><b> Fuente:</b> Baseball-Reference. <b>Actualizado en:</b> %s",
      hoy
    )
  ) +
  theme(
...
save_reels_plots <- function(plot, width = 1080, height = 1920, dpi = 300) {
  filename <- paste0("dominican_mlb_debuts_all.png")
...
save_square_plots <- function(
  ...
  for (i in seq_along(plot_list)) {
    filename <- paste0("dominican_mlb_debuts_chart_", i, ".png")
...
for (i in seq_along(plot_list)) {
  ...
  cat(
    ...
    "- Teams:",
    paste(
      team_counts_complete$Team_clean[
        ((i - 1) * teams_per_chart + 1):min(i * teams_per_chart, total_teams)
      ],
      collapse = ", "
    ),
    "\n\n"
...
# Using the rd_debut_game dataframe already created, make a time series plot of cumulative debuts over time,
# grouped by Team and using the Years of the "Date" column as the x axis

rd_debut_time_series <- rd_debut_game %>%
...
debut_time_series_plot <- rd_debut_time_series_mapped |>
  ...
  labs(
    title = "Debut de Dominicanos en MLB a lo largo del tiempo",
...
# Error in geom_mlb_logo(data = rd_debut_time_series_mapped %>% group_by(Team_clean) %>%  :
#                          could not find function "geom_mlb_logo"

debut_time_series_plot_logos <- debut_time_series_plot +
  mlbplotR::geom_mlb_logos(
    data = rd_debut_time_series_mapped |>
      dplyr::group_by(Team_clean) |>
      dplyr::filter(Year == max(Year)) |>
      dplyr::ungroup(),
    aes(x = Year + 0.5, y = Cumulative_Debuts, team_abbr = Team_clean), # <-- key aesthetic
...
ends <- rd_debut_time_series_mapped %>%
  group_by(Team_clean) %>%
